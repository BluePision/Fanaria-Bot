...
## 設計思想
- JSONファイルをデータベースとして使用せず、SQLite（`sqlite3`）を使用する
  > JSONは書き換え中に中断してしまうとファイル全体が壊れやすく、  
  > 排他制御やトランザクションがない ため。  
  > SQLiteは排他制御やトランザクションが使え、  
  > データ管理や検索に適している。
...
- 関数を大量に使ってゴリ押さず、クラスを使う
  > 状態を持つ処理や、深く関係する処理はクラスで設計する  
  > 見栄えもいい、可読性もいい、管理もしやすいため  
  > `@dataclass`と`Enum`にはお世話になりまくります
